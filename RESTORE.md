...
- `/usr/local/lib/beaconbutty/` — shared Python modules (`bb_enrich.py`, `bb_fp.py`). Also regenerated by `05_configure.sh`, but restoring them means the webapp and CLI work before a full reinstall.
...
| `/var/lib/beaconbutty/org-aliases.json` | Display-only ASN owner aliases, hand-edited (in snapshot) |
| `/var/lib/beaconbutty/device-names.json` | Manual device name overrides (in snapshot) |
| `/var/lib/beaconbutty/ip-names-cache.json` | IP→hostname cache — **NOT** in snapshot, regenerates on first run |
| `/var/lib/beaconbutty/ip-intel-cache.json` | External intel cache — **NOT** in snapshot, refetched by the daily timer |
...
| `/usr/local/lib/beaconbutty/` | Shared Python modules (in snapshot; also installed by `05_configure.sh`) |
