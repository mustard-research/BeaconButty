...
- **Zeek downgrades are no longer available** (noted 2026-09-20, on the 8.2.2 → 9.0.0 upgrade). The openSUSE repo carries only the current release per line — 9.0.0 and `zeek-lts` 8.0.10 — and 8.2.2 vanished from it while we were running it, with only 8.1.1 left in the local apt cache. There is no `apt install zeek=<previous>` to fall back on. Before any future Zeek upgrade, tar `/opt/zeek` (excluding `logs`/`spool`, ~122 MB compressed) into `/var/lib/beaconbutty/zeek-upgrade/<UTC-ts>/`: the file tree *is* the rollback. Plan B is `zeek-lts`, a different package set.
