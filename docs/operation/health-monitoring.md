...
The panel renders the **last 30 days** by default and folds anything older behind a second toggle that names what it is hiding (count, date span, total downtime). This bounds the table as years accrue without truncating the record: `outage-history.json` still keeps everything, the fold is presentation only, and the summary counts above it are computed over the whole history. Evidence written before 2026-08-31 carries no witness line — those outages were sampled by the older probe set, and no amount of re-reading recovers a measurement that was never taken.

One subtlety in how an outage gets its single class: the witnesses are **one-way and can fire on any check**, so collapsing the per-check classes by majority vote would let an outage whose ninth check finally caught a DHCP renewal be reported on the strength of the eight checks that had no evidence yet. A witnessed finding therefore wins outright (`_collapse_classes`), and the evidence summary scans **every** sample rather than only the first — reading `samples[0]` alone, as it did until 2026-08-31, would collect the witness and then never show it.
