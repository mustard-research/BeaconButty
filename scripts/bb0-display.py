...
DISPLAY_OFF_FLAG = Path("/var/lib/beaconbutty/display-off")           # content "1" = OLED blanked by user
    ...
    try:
        while running:
            try:
                ...
                # Display is on — resume normally
                if oled_blanked:
                    oled_blanked = False
                    current_contrast = None  # force contrast reapply
        ...
        # LEDs are intentionally left running so they keep breathing during
        # the shutdown/reboot sequence — the OS will kill the process when done.
        if not (DISPLAY_OFF_FLAG.exists() and DISPLAY_OFF_FLAG.read_text().strip() == "1"):
            try:
